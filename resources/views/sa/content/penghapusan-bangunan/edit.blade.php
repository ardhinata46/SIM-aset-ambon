...
<div class="py-3 d-flex flex-row align-items-center justify-content-between">
    <h1 class="h3 mb-0 text-gray-800">Ubah Data Penghapusan Aset Bangunan</h1>
    <a href="{{route('superadmin.penghapusan_bangunan.index')}}"><button class="btn btn-primary">Kembali</button></a>
</div>

<form method="POST" action="{{route ('superadmin.penghapusan_bangunan.update', $penghapusanBangunan->id_penghapusan_bangunan)}}" enctype="multipart/form-data">
    @csrf
    <div class="card mb-4">
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <input type="text" name="kode_penghapusan_bangunan" value="{{  $penghapusanBangunan->kode_penghapusan_bangunan }}" class="form-control" id="kode_penghapusan_bangunan" readonly>
                </div>
                <div class="col-md-6">
                    <label for="tanggal">Tanggal Penghapusan <span style="color: red">*</span></label>
                    <input type="date" name="tanggal" max="{{ date('Y-m-d') }}" value="{{  $penghapusanBangunan->tanggal }}" class="form-control @error('tanggal') is-invalid @enderror" id="tanggal" required>
                    @error('tanggal')
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="id_bangunan" id="id_bangunan" class="form-control  @error('id_bangunan') is-invalid @enderror" required>
                            ...
                            <option value=" {{$row->id_bangunan}}" {{ $row->id_bangunan == $penghapusanBangunan->id_bangunan ? 'selected' : '' }}>{{$row->kode_bangunan}}| {{$row->nama_bangunan}} |
                                @if($row->kondisi == 'baik')
                                Baik
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            <div class="form-check form-check-inline @error('tindakan') is-invalid @enderror">
                                <input class="form-check-input" type="radio" name="tindakan" id="jual" value="jual" {{ $penghapusanBangunan->tindakan === 'jual' ? 'checked' : '' }} required>
                                <label class="form-check-label" for="jual">Dijual</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="tindakan" id="hibah" value="hibah" {{ $penghapusanBangunan->tindakan === 'hibah' ? 'checked' : '' }} required>
                                <label class="form-check-label" for="hibah">Dihibahkan</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="tindakan" id="dihanguskan" value="dihanguskan" {{ $penghapusanBangunan->tindakan === 'dihanguskan' ? 'checked' : '' }} required>
                ...
                <div class="col-md-6">
                    <div id="harga-input" style="display: none;">
                        ...
                        <input type="number " min="0" name="harga" id="harga" value="{{ $penghapusanBangunan->harga}}" class="form-control">
                    </div>

                    <div class="form-group">
                        <label for="keterangan">Keterangan</label>
                        <textarea name="keterangan" class="form-control" id="keterangan">{{ $penghapusanBangunan->keterangan }}</textarea>
                        @error('keterangan')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
