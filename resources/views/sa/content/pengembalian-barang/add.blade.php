...
<div class="py-3 d-flex flex-row align-items-center justify-content-between">
    ...
    <a href="{{route('superadmin.pengembalian_barang.index')}}"><button class="btn btn-primary">Kembali</button></a>
</div>
<div class="row">
    <div class="col-md-5">
        <form action="{{ route('superadmin.pengembalian_barang.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="id_peminjaman_barang" id="id_peminjaman_barang" class="form-control @error('id_peminjaman_barang') is-invalid @enderror" required>
                            ...
                            <option value="{{ $row->id_peminjaman_barang }}" data-kode="{{ $row->kode_peminjaman_barang }}" data-status="{{ $row->status }}" data-tanggal="{{ $row->tanggal }}" data-nama="{{ $row->nama_peminjam }}" data-kontak="{{ $row->kontak }}" data-alamat="{{ $row->alamat }}">
