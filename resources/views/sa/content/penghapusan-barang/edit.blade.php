...
<div class="py-3 d-flex flex-row align-items-center justify-content-between">
    <h1 class="h3 mb-0 text-gray-800">Ubah Penghapusan Aset Barang</h1>
    <a href="{{route('superadmin.penghapusan_barang.index')}}"><button class="btn btn-primary">Kembali</button></a>
</div>

<form method="POST" action="{{route ('superadmin.penghapusan_barang.update', $penghapusanBarang->id_penghapusan_barang)}}" enctype="multipart/form-data">
    @csrf
    <div class="card mb-4">
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <input type="hidden" name="created_by" value="{{ $penghapusanBarang->created_by }}">
                    <label for="kode_penghapusan_barang">Kode Penghapusan Barang</label>
                    <input type="text" name="kode_penghapusan_barang" value="{{ $penghapusanBarang->kode_penghapusan_barang  }}" class="form-control" id="kode_penghapusan_barang" readonly>
                </div>
                <div class="col-md-6">
                    <label for="tanggal">Tanggal Penghapusan</label>
                    <input type="date" name="tanggal" max="{{ date('Y-m-d') }}" value="{{ $penghapusanBarang->tanggal}}" class=" form-control @error('tanggal') is-invalid @enderror" id="tanggal" autofocus required>
                    @error('tanggal')
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="id_item_barang" id="id_item_barang" class="form-control @error('id_item_barang') is-invalid @enderror" required>
                            ...
                            <option value="{{ $row->id_item_barang }}" {{ $row->id_item_barang == $penghapusanBarang->id_item_barang ? 'selected' : '' }}>{{ $row->kode_item_barang}} {{ $row->nama_item_barang }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            <div class="form-check form-check-inline @error('tindakan') is-invalid @enderror">
                                <input class="form-check-input" type="radio" name="tindakan" id="jual" value="jual" {{ $penghapusanBarang->tindakan === 'jual' ? 'checked' : '' }} required>
                                <label class="form-check-label" for="jual">Dijual</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="tindakan" id="hibah" value="hibah" {{ $penghapusanBarang->tindakan === 'hibah' ? 'checked' : '' }} required>
                                <label class="form-check-label" for="hibah">Dihibahkan</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="tindakan" id="dihanguskan" value="dihanguskan" {{ $penghapusanBarang->dihanguskan === 'hibah' ? 'checked' : '' }} required>
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            <div class="form-check form-check-inline @error('alasan') is-invalid @enderror">
                                <input class="form-check-input" type="radio" name="alasan" id="rusak" value="rusak" {{ $penghapusanBarang->alasan === 'rusak' ? 'checked' : '' }} required>
                                <label class="form-check-label" for="rusak">Rusak</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="alasan" id="hilang" value="hilang" {{ $penghapusanBarang->alasan === 'hilang' ? 'checked' : '' }} required>
                                <label class="form-check-label" for="hilang">Hilang</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="alasan" id="tidak_digunakan" value="tidak_digunakan" {{ $penghapusanBarang->alasan === 'tidak_digunakan' ? 'checked' : '' }} required>
                ...
                <div class="col-md-6">
                    <div id="harga-input" style="display: none;">
                        ...
                        <input type="text" name="harga" id="harga" value="{{ $penghapusanBarang->harga }}" class="form-control">
                    </div>
                    <div class="form-group">
